...
export function useGithubLanguages(username: string) {
     ...
     * 获取指定仓库的语言统计
     */
    const fetchRepoLanguages = async (repoName: string): Promise<LanguageStats> => {
        const response = await fetch(
    ...
    const fetchTopLanguages = async () => {
        ...
        try {
            ...
            if (repos.length === 0) {
                ...
                const languageStatsArray = await Promise.all(allRepos.map(fetchRepoLanguages))
                ...
                // 2. 并发获取这些仓库的语言统计
                const languageStatsArray = await Promise.all(repos.map(fetchRepoLanguages))
                // 3. 合并统计
                const combined: LanguageStats = {}
                for (const stats of languageStatsArray) {
                    for (const [lang, bytes] of Object.entries(stats)) {
